...
function LoginPage({ onLogin }) {
  ...
  return <div className="login-page"><div className="login-card">
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}><div className="sidebar-logo">B</div><span style={{ fontSize: 24, fontWeight: 800 }}>Brevet <span style={{ color: 'var(--indigo)' }}>Booster</span></span></div>
...
function CourseSidebar({ sections, completedVideos, completedChapters, currentPage, setPage, selectedVideo, onSelectVideo, onLogout, mobileOpen, setMobileOpen, myProf }) {
  ...
  const navStyle = (active) => ({ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', margin: '2px 12px', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: active ? 600 : 500, color: active ? 'var(--indigo)' : 'var(--text-sec)', background: active ? 'var(--indigo-bg)' : 'transparent', transition: 'all 0.15s' })
  return (
    <>
      <div className="mobile-topbar"><div onClick={() => setMobileOpen(true)} style={{ cursor: 'pointer', padding: 4 }}>{IC.menu}</div><span style={{ fontSize: 16, fontWeight: 800 }}>Brevet <span style={{ color: 'var(--indigo)' }}>Booster</span></span><div style={{ width: 26 }} /></div>
      ...
      <div className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
        ...
        {/* Section label */}
        <div onClick={() => { setPage('chapters'); setMobileOpen(false) }} style={{ padding: '12px 20px 6px', fontSize: 11, fontWeight: 700, color: currentPage === 'chapters' ? 'var(--indigo)' : 'var(--text-light)', textTransform: 'uppercase', letterSpacing: 0.8, cursor: 'pointer', transition: 'color 0.15s' }}>📖 Chapitres</div>
          ...
          {sections.map(sec => {
            ...
            return (
              <div key={sec.id} style={{ marginBottom: 2 }}>
                {/* Section header */}
                <div onClick={() => toggleSec(sec.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', margin: '2px 8px', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: 700, color: allDone ? 'var(--green-dark)' : 'var(--text)', transition: 'all 0.15s', background: allDone ? 'var(--green-bg)' : isOpen ? 'var(--indigo-bg)' : 'transparent' }}>
                      ...
                      <div key={ch.id}>
                        ...
                        {chOpen && <div style={{ paddingLeft: 20 }}>
                          {vids.map(v => {
                            ...
                            return <div key={v.id} onClick={() => { onSelectVideo(v, ch); setMobileOpen(false) }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', margin: '1px 8px', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: active ? 700 : 500, color: active ? 'white' : vDone ? 'var(--green)' : 'var(--text-sec)', background: active ? 'var(--indigo)' : 'transparent', transition: 'all 0.15s' }}>
                              {active ? <span style={{ color: 'white', flexShrink: 0 }}>{IC.play}</span> : vDone ? <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="3" style={{ flexShrink: 0 }}><polyline points="20 6 9 17 4 12"/></svg> : <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--border)', flexShrink: 0, marginLeft: 3, marginRight: 3 }} />}
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      <div className="welcome-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <div><h1 style={{ fontSize: 26, fontWeight: 800, letterSpacing: -0.3 }}>Salut {userName || 'champion'} 👋</h1><p style={{ fontSize: 15, color: 'var(--text-sec)' }}>Brevet dans <span style={{ fontWeight: 700, color: 'var(--indigo)' }}>{daysLeft} jours</span></p></div>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: '28px 24px', marginBottom: 16, textAlign: 'center' }}>
        <svg width="130" height="130" viewBox="0 0 130 130" style={{ marginBottom: 12 }}>
          ...
          <circle cx="65" cy="65" r="50" fill="none" stroke="var(--indigo)" strokeWidth="8" strokeDasharray="314" strokeDashoffset={pctStroke} strokeLinecap="round" transform="rotate(-90 65 65)" style={{ transition: 'stroke-dashoffset 1s ease' }}/>
      ...
      <div className="welcome-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
        {[['var(--indigo-bg)', 'var(--indigo-dark)', completedChapters.length, 'chapitres'], ['var(--orange-bg)', '#C2410C', (streak.current_streak || 0) + 'j', 'streak'], ['var(--green-bg)', 'var(--green-dark)', fmtTime(totalTime), 'temps passé'], ['var(--violet-bg)', '#7E22CE', completedVideos, 'vidéos vues']].map(([bg, color, val, label], i) => (
          <div key={i} style={{ background: bg, borderRadius: 14, padding: '16px 10px', textAlign: 'center' }}><div style={{ fontSize: 24, fontWeight: 900, color, lineHeight: 1 }}>{val}</div><div style={{ fontSize: 13, color, opacity: 0.7, marginTop: 6, fontWeight: 600 }}>{label}</div></div>
        ))}
      </div>
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 16 }}>{allVids.map((v, vi) => {
        ...
        return <div key={v.id} onClick={() => onSelectVideo(v, chapter)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 14px', borderRadius: 10, background: active ? 'var(--indigo-bg)' : done ? 'var(--green-bg)' : 'var(--bg)', border: active ? '1.5px solid var(--indigo-light)' : '1px solid transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
          {done && !active ? <div style={{ width: 24, height: 24, borderRadius: 7, background: 'var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg></div> : <div style={{ width: 24, height: 24, borderRadius: 7, background: active ? 'var(--indigo)' : 'var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: active ? 'white' : 'var(--text-sec)', fontSize: 11, fontWeight: 700, fontFamily: 'monospace' }}>{active ? '▶' : vi + 1}</div>}
          <span style={{ flex: 1, fontSize: 14, fontWeight: active ? 700 : 500, color: active ? 'var(--indigo)' : done ? 'var(--green)' : 'var(--text)' }}>{v.title}</span>
          {v.duration_minutes > 0 && <span style={{ padding: '2px 8px', borderRadius: 12, background: active ? 'rgba(37,99,235,0.12)' : 'var(--border)', fontSize: 11, fontWeight: 600, color: active ? 'var(--indigo)' : 'var(--text-sec)' }}>{v.duration_minutes}m</span>}
        </div>
      })}</div>
      {/* PDFs */}
      {chapter && (chapter.pdf_url || chapter.exercises_pdf_url || chapter.eval_pdf_url) && <div><div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-sec)', marginBottom: 8 }}>Ressources</div><div className="chapter-pdf-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        {chapter.pdf_url && <button onClick={() => onOpenPdf(chapter.pdf_url, chapter.title, 'cours')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 10, borderRadius: 10, border: '1.5px solid var(--indigo-light)', background: 'var(--indigo-bg)', color: 'var(--indigo)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>📘 Cours</button>}
    ...
    <div>
      ...
      {sections.map(sec => (
        <div key={sec.id} style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14, padding: '18px 22px', background: 'linear-gradient(135deg, var(--indigo-bg), #DBEAFE)', borderRadius: 14, border: '1px solid var(--indigo-light)' }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: 'linear-gradient(135deg, var(--indigo), var(--indigo-dark))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, color: 'white', boxShadow: '0 4px 12px rgba(37,99,235,0.25)', flexShrink: 0 }}>{sec.emoji}</div>
          ...
          {(sec.chapters || []).map(ch => {
            ...
            const chDone = completedChapters.includes(ch.id)
            return (
              <div key={ch.id} style={{ marginBottom: 10, background: 'var(--card)', border: isOpen ? '1.5px solid var(--indigo-light)' : '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
                ...
                {isOpen && <div style={{ borderTop: '1px solid var(--border)' }}>
                  <div style={{ padding: '12px 16px 8px' }}>
                    {vids.length > 0 ? vids.map((v, vi) => {
                      ...
                      return <div key={v.id} onClick={() => onSelectVideo(v, ch)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', marginBottom: 6, borderRadius: 10, background: done ? 'var(--green-bg)' : 'var(--bg)', cursor: 'pointer', transition: 'all 0.15s' }}>
                        {done ? <div style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg></div> : <div style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--indigo-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--indigo)', fontSize: 12, fontWeight: 800, fontFamily: 'monospace' }}>{vi + 1}</div>}
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: done ? 'rgba(5,150,105,0.1)' : 'var(--indigo-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: done ? 'var(--green)' : 'var(--indigo)' }}>{IC.play}</div>
                  ...
                  <div className="chapter-pdf-grid" style={{ padding: '8px 16px 14px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                    {[['📘', 'Cours', ch.pdf_url, 'cours', 'var(--indigo-bg)', 'var(--indigo)', 'var(--indigo-light)'], ['✏️', 'Exercices', ch.exercises_pdf_url, 'exercices', 'var(--red-bg)', 'var(--red)', 'var(--red-light)'], ['📝', 'Auto-éval', ch.eval_pdf_url, 'auto-eval', 'var(--gold-bg)', '#92400E', 'var(--gold-light)']].map(([icon, label, url, type, bg, color, border]) => (
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        <div className="card" style={{ flex: 1, minWidth: 200, padding: 24, textAlign: 'center' }}>
          <svg width="110" height="110" viewBox="0 0 110 110" style={{ marginBottom: 8 }}>
            ...
            <circle cx="55" cy="55" r="40" fill="none" stroke="var(--indigo)" strokeWidth="7" strokeDasharray="251" strokeDashoffset={pctStroke} strokeLinecap="round" transform="rotate(-90 55 55)" style={{ transition: 'stroke-dashoffset 1s ease' }}/>
        ...
        <div style={{ flex: 1, minWidth: 200, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div className="card" style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 12, color: 'var(--text-sec)' }}>Niveau {level.level}</div><div style={{ fontSize: 18, fontWeight: 800, color: 'var(--indigo)' }}>{level.name}</div></div>
          ...
          return (
            <div key={sec.id} style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 800, fontFamily: 'monospace', color: p === 100 ? 'var(--green)' : 'var(--indigo)' }}>{p}%</span>
              </div>
...
function ExamPage({ userId, earnXP }) {
  ...
  useEffect(() => { (async () => {
    const { data: q } = await supabase.from('quiz_questions').select('exam_name')
    const names = [...new Set((q || []).map(d => d.exam_name))].sort((a, b) => { const na = parseInt(a.match(/\d+/)?.[0] || 0); const nb = parseInt(b.match(/\d+/)?.[0] || 0); return na - nb })
    setExams(names)
    const { data: r } = await supabase.from('quiz_results').select('*').eq('user_id', userId).order('completed_at', { ascending: false })
    setResults(r || [])
  })() }, [userId])
...
function MathGame({ userId, type, title, emoji, gen, onBack, earnXP }) {
  ...
  if (st === 'done') return <div style={{ textAlign: 'center', padding: '60px 20px' }}><div style={{ fontSize: 56, marginBottom: 16 }}>{sc >= rec && rec > 0 ? '🎉' : '⏱️'}</div><div style={{ fontSize: 48, fontWeight: 800, fontFamily: 'monospace', color: 'var(--indigo)' }}>{sc}</div><p style={{ color: 'var(--text-sec)', fontSize: 16, marginBottom: 16 }}>bonnes réponses</p>{sc > rec && <div style={{ padding: '8px 20px', borderRadius: 12, background: 'var(--green-bg)', color: 'var(--green)', fontWeight: 700, display: 'inline-block', marginBottom: 16 }}>Nouveau record !</div>}<div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 20 }}><button className="btn btn-primary" onClick={start}>Rejouer</button><button className="btn btn-secondary" onClick={onBack}>Retour</button></div></div>
  return <div style={{ maxWidth: 500, margin: '0 auto' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}><span style={{ fontSize: 14, color: 'var(--text-sec)' }}>Score : <span style={{ color: 'var(--indigo)', fontSize: 20, fontWeight: 800 }}>{sc}</span></span><span style={{ fontSize: 28, fontWeight: 800, fontFamily: 'monospace', color: tl <= 10 ? 'var(--red)' : 'var(--text)' }}>{tl}s</span></div><GameTimer tl={tl} /><div style={{ textAlign: 'center', padding: '48px 0' }}><div style={{ fontSize: 48, fontWeight: 800, fontFamily: 'monospace', marginBottom: 24 }}>{q?.display} = ?</div><div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><input ref={ir} type="number" value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key === 'Enter' && chk()} autoFocus style={{ width: 130, padding: 14, fontSize: 24, fontWeight: 800, textAlign: 'center', borderRadius: 14, border: `3px solid ${fb === 'ok' ? 'var(--green)' : fb === 'no' ? 'var(--red)' : 'var(--border)'}`, background: fb === 'ok' ? 'var(--green-bg)' : fb === 'no' ? 'var(--red-bg)' : 'white', outline: 'none', fontFamily: 'monospace' }} /><button className="btn btn-primary" onClick={chk} style={{ padding: '14px 24px', fontSize: 16 }}>OK</button></div></div></div>
}
